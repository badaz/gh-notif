...
31. **Stale stacks: a conflict that is only noise (`stale.js`).** In a stack A → B → C, amending
    A and force-pushing, or squash-merging A (GitHub retargets B on main), leaves B and C with
    the OLD commits of A in their commit list: conflict, huge diff, nothing to review before a
    rebase. A conflict caused by *someone else's* merge into the base is legit and must stay.
    Both look identical on `mergeable` (`CONFLICTING`) and `mergeStateStatus` (`DIRTY`); the
    difference is whether the PR's commit list (`base..head`) holds **trailing commits** it
    does not own. Two stateless rules (both verified against the real API):
    - **squash-merged parent**: a commit of the list is associated with a **MERGED** PR ≠ self.
      By construction the list excludes what the base already has, so a commit tied to a merged
      PR is a leftover of a squash/rebase. ⚠️ Verified: the original commits of a squashed PR
      keep their `MERGED` association even after the branch is deleted.
    - **force-pushed parent**: the open PR whose head is our base branch
      (`baseRef.associatedPullRequests(states: OPEN)`) lists its `HeadRefForcePushedEvent`s;
      if one of their `beforeCommit` (old head) is in our commit list, we still sit on the old
      parent. ⚠️ `associatedPullRequests` is NOT usable for this case: its semantics is
      « reachable from the PR's head », so in a healthy stack A's commit is already associated
      with B, C, D… (false positive), and a commit dropped by a force-push loses every
      association (no signal). Only the timeline tells the old heads apart.
    A stateful alternative (« head unchanged + `commits.totalCount` grew ») was rejected: a
    push on B during the same poll as A's force-push masks the event, and B may then wait
    for review for days without a rebase — the misclassification would never self-correct.
    Cost: a **second GraphQL batch** (`getStaleSignals`, `stale` fragment: 100 commits × 5
    associated PRs + the parent's last 20 force-pushes) for the **CONFLICTING PRs only** —
    never in the main batch (already at risk of timing out inside GitHub, §8). A failed
    chunk → null signal → `isStaleStack` = false: **on doubt, show the row**. Effect: an
    others' PR flagged is dropped from `others` like a draft (the review request is noise
    until the rebase — it reappears by itself once the author rebases, the flag being
